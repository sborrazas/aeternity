...
-export([ pool_filter_lookup/1
        , pool_filter_add/2
        , pool_filter_remove/1
        , pool_filter_list/0
        , pool_filter_clear/0 ]).
%%
%% for testing
...
tab_copies(Mode, aec_pool_filter) ->
...
pool_filter_clear() ->
    ?t(mnesia:clear_table(aec_pool_filter)).
...
pool_filter_remove(Key) ->
    ?t(mnesia:delete({aec_pool_filter, Key})).

pool_filter_lookup(Key) ->
    ?t(case mnesia:read(aec_pool_filter, Key) of
           [#aec_pool_filter{value = Val}] ->
               Val;
           [] ->
               none
       end).
...
handle_table_errors(Tables, Mode, [{missing_table, aesc_state_cache_v2} | Tl]) ->
    ...
    handle_table_errors(Tables, Mode, Tl);
handle_table_errors(Tables, Mode, [{missing_table, aec_pool_filter = T} | Tl]) ->
