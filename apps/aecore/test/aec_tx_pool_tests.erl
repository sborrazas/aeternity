     ...
     [{"No txs in mempool",
      ...
      {"Test rejection of transactions",
       fun() ->
               ...
               %% A transaction from a blocked account should be rejected
               STx5 = a_signed_tx(PubKey3, new_pubkey(), 1, 20000),
               ok = aec_pool_filter:add(acct(PubKey3), <<"test blacklist">>),
               ?assertEqual({error, account_blocked}, aec_tx_pool:push(STx5)),
